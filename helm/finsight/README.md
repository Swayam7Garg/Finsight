...
Helm chart for deploying the FinSight personal finance application to Kubernetes.
...
# Staging
helm install finsight ./helm/finsight \
  -f ./helm/finsight/values-staging.yaml \
...
| `api.image.repository` | Image name | `finsight-api` |
...
| `web.image.repository` | Image name | `finsight-web` |
...
| `web.nextauthUrl` | NextAuth base URL | `https://finsight.example.com` |
| `web.nextPublicApiUrl` | Public API URL for browser | `https://finsight.example.com/api/v1` |
...
| `ingress.host` | Hostname | `finsight.example.com` |
| `ingress.annotations` | Ingress annotations | rate-limit, ssl-redirect, body-size |
| `ingress.tls.enabled` | Enable TLS | `true` |
| `ingress.tls.secretName` | TLS certificate secret | `finsight-tls` |
...
helm install finsight ./helm/finsight \
  ...
  --set secrets.mongodbUri=mongodb://mongo:27017/finsight
```
...
helm install finsight ./helm/finsight \
  -f ./helm/finsight/values-production.yaml \
  --set existingSecret=finsight-sealed-secrets
```
...
```bash
helm upgrade finsight ./helm/finsight -f ./helm/finsight/values-production.yaml
```

Rollback:

```bash
helm rollback finsight <revision>
```
...
```bash
# Lint
helm lint ./helm/finsight
helm lint ./helm/finsight -f ./helm/finsight/values-dev.yaml

# Dry-run render
helm template finsight ./helm/finsight \
...
helm diff upgrade finsight ./helm/finsight -f ./helm/finsight/values-production.yaml
```
