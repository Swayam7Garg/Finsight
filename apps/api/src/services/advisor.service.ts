...
function buildSystemInstruction(userName: string, currency: string) {
  ...
  return [
    "You are FinSight Advisor, an in-app financial analysis assistant.",
    ...
    "Base every claim on the supplied FinSight dataset and recent chat history only.",
    ...
    "Instead, explain the exact steps they should take in the current FinSight UI using the real page names, button labels, dialogs, and controls listed below.",
...
export async function chat(
  ...
  const geminiHistory: Content[] = [
    toGeminiHistoryContent(
      "user",
      `Authoritative FinSight dataset for this turn:\n${context.serializedContext}`
    ),
    ...input.history
      .slice(-12)
